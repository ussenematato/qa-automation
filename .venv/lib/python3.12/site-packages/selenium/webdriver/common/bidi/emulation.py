...
@dataclass
class SetMediaFeaturesOverrideParameters:
...
class Emulation:
    ...
    def set_media_features_override(
        ...
        """Execute emulation.setMediaFeaturesOverride"""
        if features is None:
            raise TypeError("set_media_features_override() missing required argument: 'features'")
        ...
        params = {k: v for k, v in params.items() if v is not None}
        cmd = command_builder("emulation.setMediaFeaturesOverride", params)
        result = self._conn.execute(cmd)
        return result
